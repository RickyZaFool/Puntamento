...
int main(){
    ...
    TLatex *LabelObservatoryTitle = new TLatex ( - 0.128, 1.05, "Observatory");
    ...
    std::vector<std::string> objectNames(0);
    ...
    while(predictedFile >> RaH >> RaM >> RaS >> DecD >> DecM >> DecS >> startTime >> endTime >> objectName ){
    ...
    for(unsigned long int i=0; i<startTimeList.size(); i++){
        ...
        double objLabelX = -(TMath::Sin(azimuthPredicted) *  (90 - altitude) ) / 90;
        double objLabelY = (TMath::Cos(azimuthPredicted) *  (90 - altitude) + 2.5) / 90;
        ...
        TLatex *objLabel = new TLatex(objLabelX, objLabelY, objectNames[i].c_str());
        objLabel->SetTextAlign(25);
        objLabel->SetTextColor(kRed+2);
    	objLabel->SetTextSize(0.015);
    	objLabel->Draw();
